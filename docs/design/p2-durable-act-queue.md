...
- 日期: 2026-09-25
- 状态: **🧜 规划冻结（愿景记录）**——宿主裁决 2026-09-25：决策流为核心，P2/P3
  细节规划暂不展开；本稿降级为长期愿景的候选方案存档（§2–§7 冻结，工作流模式
  启动时重估），架构约束与北极星见 §0.1
...
## 0.1 架构约束与北极星（宿主 2026-09-25 裁决，优先级高于本稿其余部分）

1. **引擎硬约束**：zen-engine 不支持状态在中间节点暂停和继续执行——工作流式的
   "节点挂起、事件到达后恢复"在引擎层不可行，任何 durable 方案不得假设引擎级暂停。
2. **durable 的可行形态**：只能是**自定义节点自行维持全局状态用于判断**——
   状态机制在 UDF/宿主侧（DB/外部存储），引擎每次仍是完整的同步求值；
   节点在求值时读取该状态决定分支走向。
3. **北极星**：编辑器核心当前服务于**决策流**（decision flow）；
   **工作流**（workflow，节点暂停并继续执行）是未来规划。
   **seal-editor 的长期目标是同时支持决策流与工作流**——P2（durable）与
   P3（LLM 审批流双模式）都是该愿景的组成部分。
4. **节奏**：目前以决策流为核心，P2/P3 **暂不开启具体规划细节**——本稿 §2–§7
   的队列机制是当时的一种候选方案（它不依赖引擎暂停，队列在引擎之外），
   但在工作流模式规划启动时须与"自定义节点全局状态"形态一并重估，
   不构成承诺。
