...
function tileDataGridClipboardBlock(block: string[][], rowCount: number, columnCount: number): string[][] {
  const tiled: string[][] = [];
  ...
  const body = grid
    .map(
      (line) =>
        '<tr>' +
        line
          .map((field) => '<td>' + escapeDataGridClipboardHtml(field).replace(/\r\n|\r|\n/g, '<br>') + '</td>')
          .join('') +
        '</tr>',
    )
    .join('');
 ...
 */
function getDataGridDisplayOrderedColumns<TData extends object>(table: DataGridTableInstance<TData>) {
  return [
...
function getDataGridActiveBound<TData extends object>(table: DataGridTableInstance<TData>) {
  ...
  if (focused) {
    ...
    const columnIndex = getDataGridDisplayOrderedColumns(table).findIndex((column) => column.id === focused.column.id);
...
function extendDataGridSelection<TData extends object>(
  ...
  const activeCorner = () => {
    ...
    return {
      rowId: rows[rowIndex === bound.minRowIndex ? bound.maxRowIndex : bound.minRowIndex]?.id ?? focused.row.id,
      columnId:
        allColumns[columnIndex === bound.minColumnIndex ? bound.maxColumnIndex : bound.minColumnIndex]?.id ??
        focused.column.id,
    };
  };
  const boundKey = () => {
...
function selectDataGridRegion<TData extends object>(table: DataGridTableInstance<TData>, target: DataGridPasteTarget) {
  const rows = table.getRowsInDisplayOrder();
  const columns = getDataGridDisplayOrderedColumns(table);
...
function startDataGridFillSession<TData extends object>(options: {
  ...
  const sourceCorner = (rowId: string, columnId: string | undefined) =>
    columnId
      ? viewport.querySelector(`tr[data-row-id="${CSS.escape(rowId)}"] td[data-col-id="${CSS.escape(columnId)}"]`)
      : null;
  const positionPreviewOutline = () => {
    const first = sourceRows.length ? sourceCorner(sourceRows[0]!.row.id, columns[bound.minColumnIndex]?.id) : null;
  ...
  const getExtensionTargets = () => {
    ...
    return extension.axis === 'row'
      ? {
          rowEntries: visibleRows.slice(sourceEndVisibleIndex + 1, sourceEndVisibleIndex + 1 + extension.count),
  ...
  const finish = (commit: boolean) => {
      ...
      for (const { row } of targets.rowEntries) {
        ...
        for (const { column: targetColumn, displayIndex } of targets.columnEntries) {
          ...
          if (targetColumn.id !== sourceColumn.id && targetEdit.parse) {
            ...
            const raw = sourceFormat ? sourceFormat(sourceValue, row.original) : String(sourceValue ?? '');
    ...
    selectDataGridRegion(table, {
      ...
      endRowIndex:
        extension.axis === 'row' ? targets.rowEntries[targets.rowEntries.length - 1]!.displayIndex : bound.maxRowIndex,
...
function DataGridCellSelection<TData extends object>({
  ...
  useEffect(() => {
    ...
    const root = (anchor.closest('[data-slot="data-grid"]') ?? anchor.parentElement) as HTMLElement | null;
    if (!root) return;

    // Split header/body grids render two viewports; the body one owns focus.
    const viewports = Array.from(root.querySelectorAll<HTMLElement>('[data-slot="data-grid-table-viewport"]'));
    const viewport = viewports.find((node) => node.querySelector('[data-slot="data-grid-table-body"]')) ?? viewports[0];
    if (!viewport) return;
    ...
    const focusTarget = viewport.querySelector<HTMLElement>('table[data-slot="data-grid-table"]') ?? viewport;
    ...
    const scrollFocusedIntoView = (
      ...
      requestAnimationFrame(() => {
        ...
        if (!scroller) return;
        scroller.scrollTop = ((targetRowIndex + 0.5) / rowCount) * scroller.scrollHeight - scroller.clientHeight / 2;
        requestAnimationFrame(() => {
    ...
    const requestCellEdit = (initialText?: string): boolean => {
      ...
      if (cellEdit?.control) {
        ...
        setEditorSession({
          ...
          baseline,
          label: typeof headerTitle === 'string' ? headerTitle : typeof header === 'string' ? header : cell.column.id,
        });
        return true;
      }
    ...
    const jumpFocus = (
      ...
      const rows = table.getRowsInDisplayOrder();
      const allColumns = getDataGridDisplayOrderedColumns(table);
      const selectable = allColumns.filter((column) => column.columnDef.enableCellSelection !== false);
      ...
      if (table.getRowModel().rows.length !== rows.length) {
        const rendered = new Set<string | null>(table.getRowModel().rows.map((row: { id: string }) => row.id));
        for (const rowEl of Array.from(viewport.querySelectorAll('tbody tr[data-row-id]'))) {
          rendered.add(rowEl.getAttribute('data-row-id'));
        }
        const step = clampedRowIndex >= rowIndex ? -1 : 1;
        while (clampedRowIndex !== rowIndex && rows[clampedRowIndex] && !rendered.has(rows[clampedRowIndex].id)) {
      ...
      if (extend) {
        ...
        if (bound) {
          anchorRowId = rows[rowIndex === bound.minRowIndex ? bound.maxRowIndex : bound.minRowIndex]?.id ?? anchorRowId;
          anchorColumnId =
            allColumns[columnIndex === bound.minColumnIndex ? bound.maxColumnIndex : bound.minColumnIndex]?.id ??
            anchorColumnId;
        }
    ...
    const moveFocusVisual = (direction: 'up' | 'down' | 'left' | 'right'): void => {
      ...
      const target = focused
        ? getDataGridStepTarget(table, viewport, { rowId: focused.row.id, columnId: focused.column.id }, direction)
        : null;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const horizontal = (direction: 'left' | 'right') => (rtl ? (direction === 'left' ? 'right' : 'left') : direction);
      ...
      switch (event.key) {
        ...
        case 'ArrowRight': {
          ...
          if (direction === 'down' && !event.shiftKey) {
            ...
            const lastRenderedId = renderedRows[renderedRows.length - 1]?.getAttribute('data-row-id');
            if (focused && lastRenderedId && focused.row.id === lastRenderedId && focusFirstIn(getAppendedRows()[0])) {
              event.preventDefault();
              return;
            }
          }
      ...
      if (event.key.length === 1 && !event.metaKey && !(event.ctrlKey && !event.altKey) && requestCellEdit(event.key)) {
        event.preventDefault();
        return;
      }
    ...
    const handlePaste = (event: ClipboardEvent) => {
      ...
      event.preventDefault();
      const table = getTable();
      const result = buildDataGridPasteDetails<TData>(table, parseDataGridClipboardText(text), viewport);
      if (!result) return;
    ...
    const getTwoStepControl = (target: EventTarget | null): Element | null => {
      ...
      return target.closest('button, a, select, input, textarea, [contenteditable], [role="combobox"]');
    };
    ...
    const getSelectionKey = () => {
      ...
      return `${table.getSelectedCellCount()}:${bounds.length}:${
        bound ? `${bound.minRowIndex},${bound.minColumnIndex},${bound.maxRowIndex},${bound.maxColumnIndex}` : ''
      }`;
    };
    const handleDragMouseUp = () => {
      const wasDrag =
        dragStartedOnCell && getTable().getSelectedCellCount() > 1 && getSelectionKey() !== dragStartSelectionKey;
    ...
    const selectionSubscription = table.atoms.cellSelection?.subscribe(() => {
      ...
      onCellSelectionChange({
        focused: focusedCell ? { rowId: focusedCell.row.id, columnId: focusedCell.column.id } : null,
  ...
  const closeEditorSession = (advance: DataGridEditorAdvance, refocus = true) => {
    ...
    if (advance) {
      ...
      const target =
        focused && viewportEl
          ? getDataGridStepTarget(tableNow, viewportEl, { rowId: focused.row.id, columnId: focused.column.id }, advance)
          : null;
    ...
    if (!refocus) return;
    requestAnimationFrame(() => {
      const focusEl = viewportEl?.querySelector<HTMLElement>('table[data-slot="data-grid-table"]') ?? viewportEl;
      focusEl?.focus();
...
function measureDataGridCellEditorStyle(viewport: HTMLElement): CSSProperties | null {
  ...
  const lineHeight = Number.parseFloat(cellStyle.lineHeight) || Number.parseFloat(cellStyle.fontSize) * 1.5 || 20;
    ...
    <Subscribe source={rowSelectionAtom}>
      {() => {
        ...
        return (
          ...
          >
            <span className='text-foreground text-sm font-medium'>{label ? label(count) : `${count} selected`}</span>
