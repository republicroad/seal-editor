...
function DataGridTableVirtualBody<TData extends object>({
  ...
  if (isVirtualizationEnabled) {
    ...
    centerRows.forEach((row, rowIndex) => {
      renderedRows.push(
        <MemoizedRenderedRow key={row.id} row={row} rowIndex={rowIndex} centerWindow={centerColumnWindow} />,
      );
    });
  }
...
function DataGridTableVirtual<TData extends object>({
  ...
  const loadingMoreMessage = props.fetchingMoreMessage || props.loadingMessage || i18n.labels.loading;
          ...
          {mergedHeaderGroups.map((headerGroup) => (
            <DataGridTableHeadRow key={headerGroup.id} rowId={headerGroup.id}>
              ...
              {centerColumnWindow
                ...
                : null}
              {centerColumnWindow && centerVisibleColumns.length - 1 - centerColumnWindow.end > 0 ? (
                <th
                  aria-hidden='true'
                  data-slot='data-grid-table-virtual-col-spacer'
